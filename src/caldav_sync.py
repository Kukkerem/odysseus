...
def _event_fields_from_component(comp):
    ...
    # A synced event with DTEND <= DTSTART (e.g. a single-day all-day event whose
    # source wrote DTEND equal to DTSTART) would be stored zero-duration and
    # silently dropped by the list_events overlap filter. Clamp to a positive
    # span. Lazy import: routes.calendar_routes imports this module at load time.
                ...
                try:
                    ...
                    try:
                        if access_token and _is_google_host(remote_url):
                            ...
                            # object with a per-href GET that Google 404s ("No
                            # events found."); fetch via a direct REPORT with
                            # inline calendar-data. Basic-auth Google stays on
                            # date_search below so its "needs OAuth" diagnostic
                            # still fires.
