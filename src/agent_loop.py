    ...
    if used_native and native_tool_calls:
        ...
        for j, tc in enumerate(native_tool_calls):
            ...
            result_message = {
                ...
                # Fence the result text as untrusted data (fork): upstream's
                # `metadata` below arms the internal tool gate, but
                # _sanitize_llm_messages strips metadata before the provider
                # call, so the model still sees raw tool output without this.
