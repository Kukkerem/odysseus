    ...
    def _read_email_sync(uid, folder, account_id, owner, mark_seen=False, full=False):
        ...
        try:
            with _imap(account_id, owner=owner) as conn:
                ...
                # read-only mailbox selection. Routed through _imap_select so a
                # pooled connection already on this (folder, mode) skips the
                # redundant round-trip, and a read-only selection left behind by
                # an earlier search is re-issued read-write before the STORE.
                try:
                    select_status = _imap_select(conn, folder, readonly=not mark_seen)
                    ...
                    # The same refusal also arrives as a NO/BAD status rather
                    # than an exception; _imap_select returns it verbatim.
                    if mark_seen and select_status != "OK":
                        logger.warning(
                            f"read-write SELECT for {folder!r} returned "
                            f"{select_status}; serving read-only without \\Seen"
                        )
                        _imap_select(conn, folder, readonly=True)
                        mark_seen_failed = True
