...
def test_ascii_query_uses_single_inline_search():
    ...
    assert all(f in args[1] for f in ("FROM", "TO", "CC", "SUBJECT", "TEXT"))
...
def test_non_ascii_query_uses_charset_literals_and_unions():
    ...
    # OR across every probed field == union, deduped, ascending UID order.
    # TO/CC are probed too so accented recipient searches match the ASCII path.
...
def test_non_ascii_query_with_no_hits_returns_empty():
    ...
    assert out == []
    assert len(conn.searches) == 5  # still probes every field
